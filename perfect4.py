...
with input_tabs[0]:
    ...
    st.caption("V順位は '14...' 形式（ハイフン無し）。着順は上位3まで（例: 172）。隊列は任意で '123|45|67'。4～7車のみ有効。")
    ...
    for i in range(1, 13):
        ...
        elif any([day, vorder, finish, blocks]):
            # 何か入っているのに条件不一致
            st.warning(f"R{rid}: 入力不整合（開催日/V順位/頭数を確認）。V順位は{field}桁以下、4～7車のみ対象。")

# ---------- B. 前日までの集計手入力 ----------
with input_tabs[1]:
    st.subheader("前日までの集計（開催日×ランク1～7の入賞回数）")
    st.caption("分母N_r（その開催日にVランクrが存在したレース数）と、1/2/3着回数を入力。未入力は0として扱います。")
    ...
    for day in DAY_OPTIONS:
        ...
        ch[3].markdown("2着回数")
        ch[4].markdown("3着回数")
        # 予備: V1-6/V1-7 上位2成立回数（任意）
        # ch[5].markdown("V1-6上位2")
        # ch[6].markdown("V1-7上位2")
        ch[7].markdown("備考")
        for r in range(1, 8):
...
line_oneblock_total: Dict[str, int] = defaultdict(int)  # day→件数
line_oneline_count: Dict[str, int] = defaultdict(int)   # day→一列棒状
...
with input_tabs[2]:
    ...
    for day in DAY_OPTIONS:
        ...
        tables.append((day, df_day))

    st.divider()
    st.subheader("開催日別：連対ペア分布（{i,j}：順序なし／件数と構成比%）")
    for day in DAY_OPTIONS:
        # 総分母 = その日の上位2が揃っているレース数（=ペア総数）
        ...
        st.markdown(f"**{day}**")
        st.dataframe(df_pairs[df_pairs["件数"]>0].sort_values(["件数"], ascending=False), use_container_width=True, hide_index=True)
