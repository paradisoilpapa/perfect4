...
with tabs[2]:
    ...
    # 個別2車複候補・配当収束シミュレーション設定は通常表示から削除。
    # 内部計算に必要な値だけ固定値で保持します。
    OVERHEAT_Z = 60.0
    PAY_OVERHEAT_Z = 60.0
    FINAL_POINT_N = 2
    MIN_EXPECTED_PAIR_RATE = 0.0
    NOTE_MAX_N = 3
    ROI_DIFF_LIMIT = 10.0
    PAY_RETURN_ONLY = True
    pair_base_avg_pay = dict(PAIR_BASE_AVG_PAY_DEFAULTS)
    ...
    # 最終2車複候補・1-2ゾーン基礎集計・34-12フォメ集計ゾーンは通常表示から削除。
    # ここでは 123-123-4 / 124-124-3 の比較ゾーンだけを表示します。
