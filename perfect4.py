...
st.title("ヴェロビ 復習（全体累積）｜2車複3・4軸ゾーン対応版")
...
def new_payout_rec() -> Dict[str, int]:
    # Z10 / Z20 / Z20P は2車複的中時の配当ゾーン本数。
    # 他券種では未使用でも0のままなので既存処理と互換。
    return {"N": 0, "KSUM": 0, "H": 0, "SUM": 0, "Z10": 0, "Z20": 0, "Z20P": 0}


def zone_key_from_pay(pay: int) -> str | None:
    """2車複払戻（100円あたり）から的中ゾーンキーを返す。"""
    try:
        pay = int(pay)
    except Exception:
        return None
...
def add_zone_hit(rec: Dict[str, int], pay: int) -> None:
    """的中時に払戻ゾーンを1本加算する。"""
    k = zone_key_from_pay(pay)
    if k:
        rec[k] = int(rec.get(k, 0)) + 1
...
def combine_recs(recs: List[Dict[str, int]]) -> Dict[str, int]:
    ...
    for rec in recs:
        ...
        out["Z10"] += int(rec.get("Z10", 0))
        out["Z20"] += int(rec.get("Z20", 0))
        out["Z20P"] += int(rec.get("Z20P", 0))
    return out
...
def payout_row(label: str, rec: Dict[str, int]) -> Dict:
    ...
    return {
        ...
        "10.1〜20倍": z20,
        "20.1倍〜": z20p,
        "ゾーン確認": "" if H == 0 and zsum == 0 else ("OK" if zsum == H else f"不一致({zsum}/{H})"),
        "判定": "",
    }
...
with tabs[1]:
    ...
    with st.form("prev_aggregate_form"):
        ...
        st.caption("1着が評価1〜7のとき、3着の評価の回数を入力。不要なら空欄のままでOK。")
        ...
        st.caption("対象N・払戻合計SUM・的中Hを入力。的中ゾーンは任意入力ですが、Hと合うか下の表で確認できます。")

        nh = st.columns([1.3, 1, 1.1, 1, 1, 1, 1])
        nh[0].markdown("**ペア**")
        nh[1].markdown("**対象N**")
        nh[2].markdown("**払戻SUM**")
        nh[3].markdown("**的中H**")
        nh[4].markdown("**〜10倍**")
        nh[5].markdown("**10.1〜20倍**")
        nh[6].markdown("**20.1倍〜**")
        ...
        for a, b in NISHAFUKU_PAIRS:
            ...
            key_pair = f"{a}_{b}"
            c0, c1, c2, c3, c4, c5, c6 = st.columns([1.3, 1, 1.1, 1, 1, 1, 1])
            c0.write(label.replace("2車複 ", ""))
            N = c1.number_input("", key=f"nisha_prev_N_{key_pair}", min_value=0, value=0)
            SUM = c2.number_input("", key=f"nisha_prev_SUM_{key_pair}", min_value=0, value=0, step=10)
            H = c3.number_input("", key=f"nisha_prev_H_{key_pair}", min_value=0, value=0)
            Z10 = c4.number_input("", key=f"nisha_prev_Z10_{key_pair}", min_value=0, value=0)
            Z20 = c5.number_input("", key=f"nisha_prev_Z20_{key_pair}", min_value=0, value=0)
            Z20P = c6.number_input("", key=f"nisha_prev_Z20P_{key_pair}", min_value=0, value=0)
            nishafuku_pair_inputs.append((label, int(N), int(SUM), int(H), int(Z10), int(Z20), int(Z20P)))
    ...
    for label, N, SUM, H, Z10, Z20, Z20P in nishafuku_pair_inputs:
        if any([N, SUM, H, Z10, Z20, Z20P]) and label in agg_payout_nishafuku_manual:
...
for row in byrace_rows:
    ...
    if win_rank is None or third_rank is None:
        continue
...
with tabs[2]:
    ...
    st.caption("1着評価ごとに、3着評価の分布を表示します。")

    df13_count, df13_pct = build_conditional_tables(pair13_total)
    ...
    st.caption("3-4 / 3-5 / 3-6 / 3-7 / 4-5 / 4-6 / 4-7 を追加済み。的中ゾーンは、〜10倍 / 10.1〜20倍 / 20.1倍〜 で集計します。")
    ...
    for a, b in NISHAFUKU_PAIRS:
        ...
        row = payout_row(label, rec)
        pair_key = label.replace("2車複 ", "")
        row["ペア"] = pair_key
        # 基準値があるペアだけ参考値を併記。追加ペアは空欄のままでOK。
        row["ペア基準配当"] = PAIR_BASE_AVG_PAY_DEFAULTS.get(pair_key)
        row["想定ペア的%"] = PAIR_BASE_HIT_RATE_DEFAULTS.get(pair_key)
        rows_nf.append(row)

    df_nf = pd.DataFrame(rows_nf)
    cols_nf = [
        "ペア", "型", "対象N", "総点数KSUM", "投資額換算", "払戻合計SUM",
        "的中H", "的中率%", "平均配当", "回収率%",
        "〜10倍", "10.1〜20倍", "20.1倍〜", "ゾーン確認",
        "ペア基準配当", "想定ペア的%",
    ]
    render_sortable_table(df_nf[[c for c in cols_nf if c in df_nf.columns]])
